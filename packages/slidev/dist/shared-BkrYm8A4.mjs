...
//#endregion
//#region node/syntax/markdown-it/markdown-it-draggable-image.ts
/**
...
function extractImageId(src) {
	return (src.split(/[?#]/)[0].split("/").pop() || "").replace(/\.[^.]+$/, "").replace(/[^\w-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "") || "img";
}
/**
...
*/
function MarkdownItDraggableImage(md, markdownTransformMap, options = {}) {
	...
	const defaultImageRender = md.renderer.rules.image ?? ((tokens, idx, options$1, _env, self) => self.renderToken(tokens, idx, options$1));
	md.renderer.rules.image = function(tokens, idx, options$1, env, self) {
		const token = tokens[idx];
		if ((token.attrs?.map((a) => a[0]) ?? []).some((attr) => attr.includes("v-drag"))) return defaultImageRender(tokens, idx, options$1, env, self);
		const src = token.attrGet("src") ?? "";
		const alt = token.attrGet("alt") ?? token.content ?? "";
		const title = token.attrGet("title");
		const slideNo = env.slideNo ?? 1;
		env._draggableImageIds = env._draggableImageIds ?? /* @__PURE__ */ new Map();
		const slideIds = env._draggableImageIds.get(slideNo) ?? /* @__PURE__ */ new Set();
		env._draggableImageIds.set(slideNo, slideIds);
		const baseId = `img-${extractImageId(src)}`;
		let imgId = baseId;
		let counter = 2;
		while (slideIds.has(imgId)) imgId = `${baseId}-${counter++}`;
		slideIds.add(imgId);
